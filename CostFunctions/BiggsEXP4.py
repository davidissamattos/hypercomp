"""
Implementation of the BiggsEXP4.py function
...
class BiggsEXP4(CostFunctions):
    """
    Implementation of the BiggsEXP4.py function from
    M. Jamil and X.-S. S. Yang, “A Literature Survey of Benchmark Functions For Global Optimization Problems,” Int. J. Math. Model. Numer. Optim., vol. 4, no. 2, p. 150, Aug. 2013.
       Note: function is wrong in the paper yi= exp(-ti) -5exp(-10ti)
    """
    functionProperties = {
        'minimumValue': 0,
        'optimalArms': [[1, 10,1, 5]],
        'searchSpace': [[0, 20], [0, 20],[0, 20],[0, 20]],
        'spaceType': ['uniform', 'uniform','uniform','uniform'],
        'x0': [np.random.uniform(0,20),np.random.uniform(0,20),np.random.uniform(0,20),np.random.uniform(0,20)],
    ...
    def func(self, x):
        ...
        for i in range(11):
            ...
            value = value + np.power((x3*np.exp(-ti*x1) - x4*np.exp(-ti*x2) - yi),2)
        return value
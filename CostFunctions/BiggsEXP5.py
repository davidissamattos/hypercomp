"""
Implementation of the BiggsEXP5.py function
...
class BiggsEXP5(CostFunctions):
    ...
    functionProperties = {
        'minimumValue': 0,
        'optimalArms': [[1, 10,1, 5, 4]],
        'searchSpace': [[0, 20], [0, 20],[0, 20],[0, 20],[0, 20]],
        'spaceType': ['uniform', 'uniform','uniform','uniform','uniform'],
        'x0': [np.random.uniform(0,20),np.random.uniform(0,20),np.random.uniform(0,20),np.random.uniform(0,20),np.random.uniform(0,20)],
    ...
    def func(self, x):
        ...
        for i in range(12):
            ...
            value = value + np.power((x3*np.exp(-ti*x1) - x4*np.exp(-ti*x2) +3*np.exp(-ti*x5) - yi),2)
        return value
...
__all__ = ['Bohachevsky3']


class Bohachevsky3(CostFunctions):
    """
    Implementation of the Bohachevsky3.py function from
    ...
    def func(self, x):
        ...
        x1 = x[0]
        x2 = x[1]
        value = np.power(x1,2) + 2*np.power(x2,2) - 0.3*np.cos(3*np.pi*x1 + 4*np.pi*x2) +0.3
        return value
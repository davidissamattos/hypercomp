...
class BayesOpt(Algorithm):
    ...
    def __init__(self, objective, maxfeval, acquisition_function, model_type, initial_design_type):
        ...
        if acquisition_function not in ['EI', 'LCB', 'MPI']:
            raise Exception('GPyOpt does not have acquisition function :' + str(acquisition_function))
        if model_type not in ['GP']:
            raise Exception('GPyOpt does not have model_type :' + str(model_type))
        if initial_design_type not in ['latin', 'random']:
            raise Exception('GPyOpt does not have initial_design_type :' + str(initial_design_type))

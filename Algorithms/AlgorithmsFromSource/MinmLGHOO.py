...
class MinmLGHOO():
    def __init__(self, arms_def, v1=1.0, rho=0.5, minimum_grow=1, p_vector=None):
        """
        Here we are minimizing based on the LCB
        THis is an adaptation of the mLGHOO code for minimization instead of maximization
...
## Adding values to the list
    ...
    def get_best_arm_index(self):
        """
        get the highest branch with min mean value and the min criteria
        """
        #new algorithm uses the decision criteria, old uses the mean
        best_arm_index = int(self.arm_list[np.argmin(self.arm_list[:, self.DecisionCriteria]), self.ArmIndex])
    ...
    def calculate_lcb_for_index(self, index):
        """
        ...
        if counts > 0:
            #We get the mean - lower bound
            uncertainty_bound = - np.sqrt(
                (2 * np.log(self.get_total_counts())) / float(counts))
            mean = self.arm_list[index,self.Mean]
            h = np.max(self.get_heights_for_index(index))
            max_variation_payoff = - self.v1 * np.power(self.rho, h)
            lcb_value = mean - uncertainty_bound - max_variation_payoff
        else:
            lcb_value = -np.inf
        return lcb_value
    ...
    def calculate_bound_for_index(self, index):
        ...
        :return:
        """
        lcb = self.calculate_lcb_for_index(index)
        ...
        min_bound_children = np.min(np.array([left_children_bound, right_children_bound]))
        bound = np.max(np.array([lcb, min_bound_children]))
